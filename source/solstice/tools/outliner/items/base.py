...
import solstice.tools.outliner
...
logging.config.fileConfig(solstice.tools.outliner.get_logging_config(), disable_existing_loggers=False)
logger = logging.getLogger(__name__)
logger.setLevel(solstice.tools.outliner.get_logging_level())
...
class SolsticeOutlinerAssetItem(items.OutlinerAssetItem, object):
    ...
    def _create_replace_actions(self, replace_menu):
        ...
        rig_icon = resource.ResourceManager.instance().icon('rig')
        alembic_icon = resource.ResourceManager.instance().icon('alembic')
    ...
    def _on_replace_alembic(self):
        ...
        if self._asset_node.is_alembic():
            logger.warning('You have already Alembic file of the asset loaded!')
            return False

        node_asset = self._asset_node.asset
        if not node_asset:
            logger.warning('Impossible to retrieve Asset linked to Alembic. Aborting operation ...')
            return False
        ...
        if not abc_file_type:
            logger.warning('Alembic Asset File Type not found for Asset: {}!'.format(node_asset))
            return
        ...
        if self._asset_node.is_rig():
            ...
            if not main_node:
                logger.warning('Tag Data node: {} is not linked to any node! Aborting operation ...'.format(n_tag.node))
                return
            ...
            if attrs and type(attrs) == list:
                if 'root_ctrl' in attrs:
                    ...
                    if not tp.Dcc.object_exists(root_ctrl):
                        logger.warning('Root Control "{}" does not exists in current scene! Aborting operation ...'.format(root_ctrl))
                        return False
            ...
            if not node_to_retrieve_xform or not tp.Dcc.object_exists(node_to_retrieve_xform):
                logger.warning('No valid node found to retrieve rig transform from. Aborting operation ...')
                return
        ...
        if not is_referenced:
            ...
            for child in children:
                ...
                if not is_child_ref:
                    ...
                    break
            if not valid_refs:
                logger.warning('Impossible to replace {} by rig file ...'.format(self._asset_node.node))
                return False

            node_asset = self._asset_node.asset
            if not node_asset:
                logger.warning('Impossible to retrieve Asset linked to rig. Aborting operation ...')
                return False
            ...
            if find_root_ctrl:
                for n in ref_nodes:
                    if n.endswith('root_ctrl'):
                        ...
                        self.replaceCompleted.emit()
                        return True
                logger.warning('No Root Control found. Impossible to place rig in proper location!')
                return False
